...
# %%
params = setup_qflow_inputs(n_elec=6, norb=6)
...
# %%
def read_fci_matrix(filename, dim_fci,n_elec):
    """
...
matrix = read_fci_matrix("FCI_matrix.dat", dim_fci, n_elec)
...
# Sanity check: compute expected FCI dimension from (n_elec, n_orb)
n_alpha = n_elec // 2
n_beta = n_elec // 2
expected_dim = comb(norb, n_alpha) * comb(norb, n_beta)

assert dim_fci == expected_dim, (
    f"Mismatch: strings.shape[0] = {dim_fci}, "
    f"but expected FCI dimension = {expected_dim} "
    f"for {n_alpha}α, {n_beta}β electrons in {norb} orbitals."
)
...
#             for o in [oi, oj]:
#                 nactspin[, da, o] = nactspin[a, o + noas] = 0
#             for v in [vi, vj]:
...
with open("nactspin_fortran.dat", "r") as f:
    for line in f:
...
def initial_guess_zero(nos, nvs):
    t1 = np.zeros((nos, nvs),dtype=np.float64)
    t2 = np.zeros((nos, nos, nvs, nvs), dtype=np.float64)
    t3 = np.zeros((nos, nos, nos, nvs, nvs, nvs), dtype=np.float64)
    t4 = np.zeros((nos, nos, nos, nos, nvs, nvs, nvs, nvs), dtype=np.float64)

    return t1, t2, t3, t4
t1, t2, t3, t4 = initial_guess_zero(nos, nvs)
#print(t1[0])

# %%

def m_t_ext(m, actspin, t1, t2, t3, t4, nos, nvs):
    """
...
# %%
@njit
def compute_diff_and_indices(string1, string2):
    n = string1.shape[0]
    diff = np.empty(n, dtype=np.int8)
    idiff = 0
    pos = []
    ...
    for i in range(n):
        ...
        if d == 1:
            ...
            neg.append(i)
            idiff += 1
    return idiff // 2, diff, np.array(pos), np.array(neg)


# %%
from collections import defaultdict

# def precompute_excitation_pairs(strings, max_excitation=4):
#     """
#     Precompute determinant pairs (i, j) categorized by excitation rank (1 to max_excitation).
#     Returns:
#         excitation_map: dict {excitation_rank: [(i, j), ...]}
#     """
#     dim_fci = len(strings)
#     excitation_map = defaultdict(list)
...
#             if 1 <= idiff <= max_excitation:
#                 excitation_map[idiff].append((i, j))
#     return excitation_map
from functools import lru_cache


# def precompute_excitation_pairs(strings_np, max_excitation=4):
#     dim_fci = strings_np.shape[0]
#     excitation_map = [[] for _ in range(max_excitation + 1)]  # index 0 unused

#     for i in range(dim_fci):
#         for j in range(dim_fci):
#             idiff, _ = get_idiff_and_diff(strings_np[i], strings_np[j])
#             if 1 <= idiff <= max_excitation:
#                 excitation_map[idiff].append((i, j))
#     return excitation_map
...
def get_idiff_fast(string1, string2):
    diff = string1 - string2
    idiff = np.count_nonzero(diff) // 2
    return idiff

def precompute_excitation_pairs_parallel(strings_np, max_excitation=4, n_jobs=-1):
    """
    Parallel precomputation of determinant excitation pairs (i, j) grouped by excitation rank.
    Uses joblib to parallelize the outer loop.
    
    Args:
        strings_np: np.ndarray (dim_fci, n_orb)
        max_excitation: max excitation order (1–4)
        n_jobs: # of parallel workers (-1 = all)
        
    Returns:
        excitation_map: list of lists of (i, j) pairs
                        excitation_map[k] contains all (i,j) with k-tuple excitations
    """
    dim_fci = strings_np.shape[0]
    excitation_map = [[] for _ in range(max_excitation + 1)]

    def compute_row(i):
        row = []
        for j in range(dim_fci):
            idiff = get_idiff_fast(strings_np[i], strings_np[j])
            if 1 <= idiff <= max_excitation:
                row.append((idiff, i, j))
        return row

    results = Parallel(n_jobs=n_jobs)(delayed(compute_row)(i) for i in range(dim_fci))

    # Flatten and group by excitation rank
    for row in results:
        for idiff, i, j in row:
            excitation_map[idiff].append((i, j))

    return excitation_map

# Convert strings array to a hashable key
def strings_key(strings_np):
    return hash(strings_np.tobytes())
_cached_excitation_maps = {}

def precompute_excitation_pairs_cached(strings_np, max_excitation=4):
    key = strings_key(strings_np)
    if key in _cached_excitation_maps:
        return _cached_excitation_maps[key]

    # Compute and store the result
    excitation_map = precompute_excitation_pairs_parallel(strings_np, max_excitation=max_excitation)
    _cached_excitation_maps[key] = excitation_map
    return excitation_map
...
def m_t_ext_exp(mt1, mt2, mt3, mt4,
                nos, nvs, noas, nobs, nvas, nvbs, nstot,
                dim_fci, strings, actspin,
                debug=False):
    from scipy.sparse import csr_matrix

    # Use sparse format for efficiency in filling
    tm = lil_matrix((dim_fci, dim_fci), dtype=np.float64)
    ...
    excitation_map = precompute_excitation_pairs_cached(strings_np)

    for idiff in range(1, 5):
        for i, j in excitation_map[idiff]:
            string1 = strings_np[i]
            string2 = strings_np[j]
            _, diff, pos, neg = compute_diff_and_indices(string1, string2)
            # _, diff = get_idiff_and_diff(string1, string2)
            ...
            if idiff == 1:
                ...
                if p in virtual_map and q in occupied_map:
                    ...
                    tm[i, j] += iphase * mt1[oq, vp]

            elif idiff == 2:
                p, q = pos
                r, s = neg
                if all(k in virtual_map for k in (p, q)) and all(k in occupied_map for k in (r, s)):
                    vp, vq = virtual_map[p], virtual_map[q]
                    or_, os = occupied_map[r], occupied_map[s]
                    iphase = compute_phase(string2, [p, q], [r, s])
                    tm[i, j] += iphase * mt2[or_, os,vp, vq]

            elif idiff == 3:
                p, q, r = pos
                s, t, u = neg
                if all(k in virtual_map for k in (p, q, r)) and all(k in occupied_map for k in (s, t, u)):
                    vp, vq, vr = virtual_map[p], virtual_map[q], virtual_map[r]
                    os_, ot, ou = occupied_map[s], occupied_map[t], occupied_map[u]
                    iphase = compute_phase(string2, [p, q, r], [s, t, u])
                    tm[i, j] += iphase * mt3[os_, ot, ou, vp, vq, vr]

            elif idiff == 4:
                p, q, r, s = pos
                t, u, v, w = neg
                if all(k in virtual_map for k in (p, q, r, s)) and all(k in occupied_map for k in (t, u, v, w)):
                    vp, vq, vr, vs = virtual_map[p], virtual_map[q], virtual_map[r], virtual_map[s]
                    ot, ou, ov, ow = occupied_map[t], occupied_map[u], occupied_map[v], occupied_map[w]
                    iphase = compute_phase(string2, [p, q, r, s], [t, u, v, w])
                    tm[i, j] += iphase * mt4[ot, ou, ov, ow, vp, vq, vr, vs]

    # Convert tm to CSR for fast arithmetic and form sigma = tm - tm.T
    tm = tm.tocsr()
    sigma = tm - tm.transpose()
    sigma_dense = sigma.toarray()

    if debug:

        print("Max abs(sigma):", np.max(np.abs(sigma_dense)))
        print("Frobenius norm of sigma:", np.linalg.norm(sigma_dense))
        print("Non-zero σ elements:", sigma.nnz, "/", sigma.shape[0]**2)
        print("‖σ + σᵀ‖ =", np.linalg.norm(sigma_dense + sigma_dense.T))
  
    ept, emt = expm(sigma_dense, dim_fci, max_order=22, tol=1e-10, debug=debug)
    ...
    return ept, emt, sigma_dense, deviation
...
def optimized_hierarchy_excitations(nos, nvs, nactspin,nactives):
    ...
    it3 = np.full((nos, nos,nos, nvs, nvs, nvs), -1, dtype=int)
    it4 = np.full((nos, nos,nos, nos, nvs, nvs,nvs, nvs), -1, dtype=int)
            ...
            for j in range(nos):
                for k in range(nos):
                    for a in range(nvs):
                        for b in range(nvs):
                            for c in range(nvs):
                                if (nactspin[m, i] == 0 and nactspin[m, j] == 0 and nactspin[m, k] == 0 and
                                    nactspin[m, nos + a] == 0 and nactspin[m, nos + b] == 0 and nactspin[m, nos + c] == 0):
...
it1, it2, it3, it4 = optimized_hierarchy_excitations(nos, nvs, nactspin, nactives)
...
# %%
@njit
def mnum1234(m, actspin, it1, it2, it3, it4, spin_array, nos, nvs):
    """
    ...
    return mnum1, mnum2, mnum3, mnum4
...
def create_mlists_xm(m, actspin, it1, it2, it3, it4, t1, t2, t3, t4,
                                spin_array, nos, nvs, mnum1max, mnum2max, mnum3max, mnum4max, dim_m_max):
    ...
    m_list3 = np.zeros((mnum3max, 6), dtype=int)
    m_list4 = np.zeros((mnum4max, 8), dtype=int)
    xm = np.zeros((dim_m_max,), dtype=np.float64)
    ...
    return (
        ...
        m_list4[:mnum4],
        xm[:dim_m],
        mnum1, mnum2, mnum3, mnum4, dim_m
    )


# %%
@njit
def m_t_int(m, actspin, t1, t2, t3, t4, nos, nvs):
    """
...
# %%
def m_excitation(mt1, mt2, mt3, mt4, nos, nvs, dim_fci, strings):
    strings_np = np.array(strings)
    excitation_map = precompute_excitation_pairs_cached(strings_np)
    tm = np.zeros((dim_fci, dim_fci), dtype=np.float64)
...
#     # Finalize sparse antisymmetric matrix
#     tm = tm.tocsr()
#     sigma = tm - tm.transpose()
    
#     return sigma.toarray() if isinstance(sigma, csr_matrix) else sigma
...
def x_fan_out_int(xm, mt1, mt2, mt3, mt4,
                  mnum1, mnum2, mnum3, mnum4,
                  m_list1, m_list2, m_list3, m_list4,
    ...
    # Final check
    assert xm.shape[0] == mnum1 + mnum2 + mnum3 + mnum4, "xm length mismatch"
    return mt1, mt2, mt3, mt4
...
def fn_m(dim_fci, dim_m,
    ...
    #print("matrix_aux[0,:5]:", matrix_aux[0, :5])
    ...
    # if debug:
    #     print("⟨ψ|H_eff|ψ⟩ =", dot_val)
    #     print("‖σ + σᵀ‖ (should be ~0):", deviation)
    # return dot_val
    return energy
...
def update_global_amplitudes(t1, t2, t3, t4,
                             xm, mnum1, mnum2, mnum3, mnum4,
                             m_list1, m_list2, m_list3, m_list4,
                             dim_m, actspin, nos, nvs):
    """
...
# %%
@njit
def zero_t1234(mt1, mt2, mt3, mt4, nos, nvs):
    """
    ...
    # mt3[i, j, k, a, b, c]
    mt3[:nos, :nos, :nos, :nvs, :nvs, :nvs] = 0.0

    # mt4[i, j, k, l, a, b, c, d]
    mt4[:nos, :nos, :nos, :nos, :nvs, :nvs, :nvs, :nvs] = 0.0
...
def mpraxis(T0, MACHEP, H0, PRIN,
    ...
    aux3 = np.zeros_like(mt3)
    aux4 = np.zeros_like(mt4)
    ...
    for p, (i, ia_global) in enumerate(m_list1):
        ...
        gradient[p] = fn_m(dim_fci, dim_m, mnum1, mnum2, mnum3, mnum4,
                           mnum1max, mnum2max, mnum3max, mnum4max, dim_m_max,
                           nos, nvs, noas, nobs, nvas, nvbs, nstot,
                           trial, cm, mt1, mt2, mt3, mt4,
                           m_list1, m_list2, m_list3, m_list4, x,
                           strings, actspin)
        #print(f"[m] grad[{offset + p}] = {gradient[offset + p]:.6e}")
    ...
    #print(f"[m] max |gradient| = {np.max(np.abs(gradient)):.3e}")



    # --- Simple Gradient Descent ---
    xene_old = fn_m(dim_fci, dim_m, mnum1, mnum2, mnum3, mnum4,
                    mnum1max, mnum2max, mnum3max, mnum4max, dim_m_max,
                    nos, nvs, noas, nobs, nvas, nvbs, nstot,
                    trial, matrix, mt1, mt2, mt3, mt4,
                    m_list1, m_list2, m_list3, m_list4, x,
                    strings, actspin)

    for r in range(max_steps):
        x_new = x - dstep * gradient
        xene = fn_m(dim_fci, dim_m, mnum1, mnum2, mnum3, mnum4,
                    mnum1max, mnum2max, mnum3max, mnum4max, dim_m_max,
                    nos, nvs, noas, nobs, nvas, nvbs, nstot,
                    trial, matrix, mt1, mt2, mt3, mt4,
                    m_list1, m_list2, m_list3, m_list4, x_new,
                    strings, actspin)
        #print(f"[m] Energy at step {r}: {xene:.10f}")

        if xene < xene_old:
            for i in range(dim_m):
                 x[i] = x_new[i]  # safer explicit elementwise update
            x = x_new.copy()
            xene_old = xene
        else:
            break
    ...
    fmin = xene_old
    #print(f"Final energy from fn_m: {xene_old:.10f}")
...
def run_qflow_loop(
    ...
    for i in range(maxiter):
        ...
        for m in range(nactives):
            ...
            mt1, mt2, mt3, mt4 = m_t_ext(m, actspin, t1, t2, t3, t4,
                                          nos, nvs)
    ...
    #         # Step 2: Matrix exponentials
            ept, emt, sigma, deviation = m_t_ext_exp(mt1, mt2, mt3, mt4,
                                    nos, nvs, noas, nobs, nvas, nvbs, nstot,
                                    dim_fci, strings, actspin)
            #Fprint("‖σ + σᵀ‖ (should be ~0):", np.linalg.norm(sigma + sigma.T))
            # print(">>> Checking mt1–mt4 before sigma construction:")
    ...
    #         # # Step 4: Count internal excitation terms
            mnum1, mnum2, mnum3, mnum4 = mnum1234(m, actspin, it1, it2, it3, it4, spin_array, nos, nvs)
            #dim_m = mnum1 + mnum2 + mnum3 + mnum4
    ...
    #         # Step 4–5: Create m_lists, excitation counts, and xm
            m_list1, m_list2, m_list3, m_list4, xm, mnum1, mnum2, mnum3, mnum4, dim_m = create_mlists_xm(m, actspin, it1, it2, it3, it4, t1, t2, t3, t4,
                      spin_array, nos, nvs, mnum1max, mnum2max, mnum3max, mnum4max, dim_m_max)
    ...
    #         # Step 6: Internal T amplitudes
            mt1, mt2, mt3, mt4 = m_t_int(m, actspin, t1, t2, t3, t4, nos, nvs)
            # print("mt1 sum (py):", np.sum(mt1))
            # print("mt2 sum (py):", np.sum(mt2))
            # print("mt3 sum (py):", np.sum(mt3))
            # print("mt4 sum (py):", np.sum(mt4))
...
# Optional: print total number of elements for checking consistency with xm
    ...
    #         # Step 7: Run minimization
            energy, xm_opt,_ = mpraxis(
                 ...
                 m_list1=m_list1, m_list2=m_list2, m_list3=m_list3, m_list4=m_list4,
                 mnum1=mnum1, mnum2=mnum2, mnum3=mnum3, mnum4=mnum4,
                 mnum1max=mnum1max, mnum2max=mnum2max,
                 mnum3max=mnum3max, mnum4max=mnum4max, dim_m_max=dim_m_max,
                 nos=nos, nvs=nvs, noas=noas, nobs=nobs, nvas=nvas, nvbs=nvbs, nstot=nstot,
                 trial=trial, matrix=matrix, 
                 strings=strings, actspin=actspin, m_excitation=m_excitation,
                 commutator=commutator, fn_m=fn_m,
                 verbose=verbose)
            energy_list.append(energy)

        #     print("mt1[:5]:", mt1[:5])
        #     print("ept[0,:5]:", ept[0, :5])
        #     print("emt[0,:5]:", emt[0, :5])
        #     print("sigma[0,:5]:", sigma[0, :5])
...
#     #         # Step 8: Update global amplitudes
            update_global_amplitudes(t1, t2, t3, t4,
                         xm_opt, mnum1, mnum2, mnum3, mnum4,
                         m_list1, m_list2, m_list3, m_list4,
                         dim_m, actspin, nos, nvs)
...
# print("m_t_ext_exp:", m_t_ext_exp.__module__)
# print("m_excitation:", m_excitation.__module__)
# print("fn_m:", fn_m.__module__)
