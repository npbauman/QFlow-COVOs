"""
QFlow-SD: singles-and-doubles (T1, T2) variant of the QFlow correlation
energy method.

These functions are carried over from the original ``SD-NoTQ.py``
script with NO numerical logic changes. The only modification made
during refactoring is making the checkpoint output directory in
``run_qflow_loop`` a parameter (``outdir``) instead of a hardcoded
relative path, so the function behaves consistently regardless of the
caller's working directory; its default value ("amplitudes_ckpt")
matches the original script's hardcoded value exactly, so behavior is
unchanged unless a caller explicitly overrides it.
"""
...
#     #         # Step 8: Update global amplitudes
            update_global_amplitudes(t1, t2,
                         ...
                         m_list1, m_list2,
                         dim_m, actspin, nos, nvs)
            save_global_pool(i+1, t1, t2, outdir=outdir, prefix="amps", save_dense=False)
