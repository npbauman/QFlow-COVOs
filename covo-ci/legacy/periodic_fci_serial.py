...
def fermionic_phase_single(p_add, q_remove, string):
    """Phase for |string> -> a†_{p_add} a_{q_remove} |string>  (single flip)."""
    tmp = string.copy()
    isum = int(np.sum(tmp[:q_remove]))  # annihilate at q_remove
    tmp[q_remove] = 0
    isum += int(np.sum(tmp[:p_add]))    # create at p_add
    return -1 if (isum % 2) else 1

# --- helper: build a map from basis determinant tuple -> index (you already have this) ---
# str2idx = {tuple(s): i for i, s in enumerate(fci_strings)}

def splus_terms_of_det(det, noccp, nvirt):
    """
    Return dict {tuple(new_string): coeff} for S+|det>.
    Works in the FULL spin space (no projection).
    """
    out = {}
    norb = noccp + nvirt
    for p in range(norb):
...
def apply_op_to_vec_return_dict(vec, fci_strings, term_fn):
    """
    Apply a determinant -> {det': coeff} operator (S+ or S-) to a CI vector,
    but keep the result as a dict over determinant tuples (NOT projected).
    """
    out = {}
...
def project_detdict_to_basis(detdict, str2idx, dim):
    """
    Project a dict over determinant tuples onto your basis: returns a length-dim vector.
    Missing tuples are just dropped (i.e., they’re outside the basis).
    """
...
def s2_expectation_correct(vec, fci_strings, noccp, nvirt):
    """
    ⟨S^2⟩ = ⟨Sz^2⟩ + 1/2 (⟨ψ|S+S-|ψ⟩ + ⟨ψ|S-S+|ψ⟩)
    Evaluate without losing intermediate sectors.
    """
    dim = len(fci_strings)
    str2idx = {tuple(s): i for i, s in enumerate(fci_strings)}

    # Sz for fixed Nα, Nβ:
    Ms = 0.5 * (noccp - noccp)  # = 0 here
    sz2 = Ms * Ms
    ...
    # v1 = S- S+ |ψ|
    # First S+ to dict (lives in Ms=+1), then S- back, then project to basis
    v_sp_dict   = apply_op_to_vec_return_dict(vec, fci_strings, Splus_terms)
    # apply S- to each intermediate determinant (still dict form)
    v_sms_p_dict = {}
    for tup, amp in v_sp_dict.items():
        det = np.array(tup, dtype=int)
        terms = Sminus_terms(det)
        for tup2, coeff2 in terms.items():
...
def s_from_s2(s2, tol=1e-8):
    # Solve S(S+1) = s2 → S = (-1 + sqrt(1 + 4*s2)) / 2
    root = 1.0 + 4.0 * max(s2, 0.0)
    S = (-1.0 + np.sqrt(root)) / 2.0
    # snap to nearest allowed spin (avoid 1.999999 → 2.0)
    S_round = round(S + 0.0, 8)
    return float(S_round)
...
def diagonalize_fci_matrix_with_spin(matrix, fci_strings, noccp, nvirt, k=5):
    # Small shift to avoid accidental non-positive definiteness issues
...
# Use this instead of your previous call:
#vals, vecs = diagonalize_fci_matrix_with_spin(fci_matrix, fci_strings, noccp, nvirt, k=5)
# ----------------------------
