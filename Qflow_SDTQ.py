...
#print("Determinant 1:  ",strings[0])
#print("Determinant 2:  ",strings[1])
#print(strings[4])
#print(strings[127])
...
def act_space_size_from_actspin(strings, ref, actspin):
    maps = [i for i in range(len(strings)) if np.all((strings[i] == ref) | (actspin == 0))]
    return len(maps), np.array(maps, dtype=int)
...
def act_matrix_full(strings, actspin, matrix, ref=None):
    ...
    return {
        ...
        "matrix_a": matrix_a,
        "eigvals": eigvals,
        "lowest_energy": float(np.min(eigvals)),
    }
...
def m_t_ext(m, actspin, t1, t2, t3, t4, nos, nvs):
    ...
    Everything is zero except entries that touch at least one inactive orbital.
    """
    ...
    # T2
    mt2_mask = (
        (occ_mask[:, None] | occ_mask[None, :])[:, :, None, None] |
        (virt_mask[:, None] | virt_mask[None, :])[None, None, :, :]
    )
...
def m_t_ext_exp(mt1, mt2,mt3, mt4,
    ...
    deviation = np.linalg.norm(ept @ emt - np.eye(dim_fci))
...
def mnum12(m, actspin, it1, it2, it3, it4, spin_array, nos, nvs):
    """
...
# def save_global_pool(iter_idx, t1, t2, t3, t4,
#                      outdir="amplitudes_ckpt", prefix="amps",
#                      save_dense=False):  # keep your API
#     import os, json, time, tempfile
#     import numpy as np
...
#     # --- enforce dtype for consistency ---
#     t1 = np.asarray(t1, dtype=np.float64)
#     t2 = np.asarray(t2, dtype=np.float64)
#     t3 = np.asarray(t3, dtype=np.float64)
#     t4 = np.asarray(t4, dtype=np.float64)

#     # ---------- NPZ (atomic) ----------
#     npz_tmp = tempfile.NamedTemporaryFile(delete=False, dir=outdir, suffix=".npz")
#     try:
#         np.savez_compressed(npz_tmp, t1=t1, t2=t2, t3=t3, t4=t4, iter=int(iter_idx))
...
#     # ---------- JSON summary (atomic) ----------
#     def _nan_inf_counts(x):
#         return int(np.isnan(x).sum()), int(np.isinf(x).sum())

#     t1_nan, t1_inf = _nan_inf_counts(t1)
#     t2_nan, t2_inf = _nan_inf_counts(t2)
#     t3_nan, t3_inf = _nan_inf_counts(t3)
#     t4_nan, t4_inf = _nan_inf_counts(t4)
...
#         "t3_shape": list(t3.shape),
#         "t4_shape": list(t4.shape),
#         "t1_nonzero": int(np.count_nonzero(t1)),
#         "t2_nonzero": int(np.count_nonzero(t2)),
#         "t3_nonzero": int(np.count_nonzero(t3)),
#         "t4_nonzero": int(np.count_nonzero(t4)),
#         "t1_max_abs": float(np.max(np.abs(t1))) if t1.size else 0.0,
#         "t2_max_abs": float(np.max(np.abs(t2))) if t2.size else 0.0,
#         "t3_max_abs": float(np.max(np.abs(t3))) if t3.size else 0.0,
#         "t4_max_abs": float(np.max(np.abs(t4))) if t4.size else 0.0,
#         "t1_nan": t1_nan, "t1_inf": t1_inf,
#         "t2_nan": t2_nan, "t2_inf": t2_inf,
#         "t3_nan": t3_nan, "t3_inf": t3_inf,
#         "t4_nan": t4_nan, "t4_inf": t4_inf,
...
#     # ---------- Optional sparse TXT (canonical one-sided) ----------
#     if not save_dense:
...
#             for i_, a_ in zip(nz_i, nz_a):
#                 f.write(f"T1 {i_} {a_} {t1[i_, a_]:.16e}\n")

#             # T2: i<j, a<b
#             nos_, _, nvs_, _ = t2.shape
#             for i_ in range(nos_):
#                 for j_ in range(i_+1, nos_):
#                     for a_ in range(nvs_):
#                         for b_ in range(a_+1, nvs_):
#                             val = t2[i_, j_, a_, b_]
#                             if val != 0.0:
#                                 f.write(f"T2 {i_} {j_} {a_} {b_} {val:.16e}\n")

#             # T3: i<j<k, a<b<c
#             nos3a, nos3b, nos3c, nvs3a, nvs3b, nvs3c = t3.shape
#             for i_ in range(nos3a):
#                 for j_ in range(i_+1, nos3b):
#                     for k_ in range(j_+1, nos3c):
#                         for a_ in range(nvs3a):
#                             for b_ in range(a_+1, nvs3b):
#                                 for c_ in range(b_+1, nvs3c):
#                                     val = t3[i_, j_, k_, a_, b_, c_]
#                                     if val != 0.0:
#                                         f.write(f"T3 {i_} {j_} {k_} {a_} {b_} {c_} {val:.16e}\n")

#             # T4: i<j<k<l, a<b<c<d
#             nos4a,nos4b,nos4c,nos4d,nvs4a,nvs4b,nvs4c,nvs4d = t4.shape
#             for i_ in range(nos4a):
#                 for j_ in range(i_+1, nos4b):
#                     for k_ in range(j_+1, nos4c):
#                         for l_ in range(k_+1, nos4d):
#                             for a_ in range(nvs4a):
#                                 for b_ in range(a_+1, nvs4b):
#                                     for c_ in range(b_+1, nvs4c):
#                                         for d_ in range(c_+1, nvs4d):
#                                             val = t4[i_, j_, k_, l_, a_, b_, c_, d_]
#                                             if val != 0.0:
#                                                 f.write(f"T4 {i_} {j_} {k_} {l_} {a_} {b_} {c_} {d_} {val:.16e}\n")
#             f.flush(); os.fsync(f.fileno())
...
# %%
@njit
def zero_t12(mt1, mt2,mt3, mt4, nos, nvs):
    """
...
def run_qflow_loop(
    ...
    #         # # Step 4: Count internal excitation terms
            mnum1, mnum2, mnum3, mnum4 = mnum12(m, actspin, it1, it2, it3, it4, spin_array, nos, nvs)

    #         # Step 4–5: Create m_lists, excitation counts, and xm
            m_list1, m_list2, m_list3, m_list4, xm, mnum1, mnum2,mnum3, mnum4, dim_m = create_mlists_xm(m, actspin, it1, it2, it3, it4, 
                    t1, t2, t3, t4, spin_array, nos, nvs, mnum1max, mnum2max,mnum3max, mnum4max, dim_m_max)
...
# Optional: print total number of elements for checking consistency with xm
    ...
    #         # Step 7: Run minimization
            energy, xm_opt,_ = mpraxis(
                 ...
                 mnum1max=mnum1max, mnum2max=mnum2max,mnum3max=mnum3max, mnum4max=mnum4max,
                 dim_m_max=dim_m_max,
...
#     #         # Step 8: Update global amplitudes
            update_global_amplitudes(t1, t2,t3, t4,
                         ...
                         m_list1, m_list2, m_list3, m_list4,
                         dim_m, actspin, nos, nvs)
            #save_global_pool(i+1, t1, t2, t3, t4, outdir="amplitudes_ckpt", prefix="amps", save_dense=False)
